...
pub trait Bus {
    ...
    fn write(&mut self, addr: Self::Addr, val: Self::Data);
}
...
pub trait ReadFromBytes : Bus<Data=u8> {
    ...
    fn read_word<O: ByteOrder>(&self, addr: Self::Addr) -> u16 {
        let data = [self.read(addr), self.read(addr + Self::Addr::one())];
        O::read_u16(&data)
    }
}
...
pub trait WriteFromBytes : Bus<Data=u8> {
    ...
    fn write_word<O: ByteOrder>(&mut self, addr: Self::Addr, val: u16) {
        let mut data = [0; 2];
        O::write_u16(&mut data, val);
        self.write(addr, data[0]);
        self.write(addr + Self::Addr::one(), data[1]);
    }
}
impl<T> WriteFromBytes for T where T: Bus<Data=u8> {}

// Read a 16-bits word from a 8-bits bus
pub fn read_word<O, B>(bus: &B, addr: B::Addr) -> u16 
where B: Bus<Data=u8>, O: ByteOrder {
    let data = [bus.read(addr), bus.read(addr + B::Addr::one())];
    O::read_u16(&data)
}

// Write a 16-bits word into a 8-bits bus
pub fn write_word<O, B>(bus: &mut B, addr: B::Addr, val: u16) 
where B: Bus<Data=u8>, O: ByteOrder {
    let mut data = [0; 2];
    O::write_u16(&mut data, val);
    bus.write(addr, data[0]);
    bus.write(addr + B::Addr::one(), data[1]);
}
